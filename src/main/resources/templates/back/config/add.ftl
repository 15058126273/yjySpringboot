...
<body>
<form class="rightBox" action="${backBase}/config/save.do" method="post" id="configFrom">
    <div class="detailSection">
        <ul>
            <li><span>*</span>参数名称：</li>
            <li>
                <input type="text" name="fieldName" class="required" size="45" class="" value="${(bean.fieldName)!}"
                       placeholder="输入参数名称"/>
            </li>
        </ul>
    </div>
            ...
            <li>
                <select name="status" class="required via">
                 		<#list Application['commonStatus'> as item >
                            <option value="${(item.fieldKey)!}" <#if bean?? && bean.status?? && item.fieldKey == bean.status >
