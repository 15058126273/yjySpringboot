...
<body>
<form class="rightBox" action="${backBase}/optionItem/update.do" method="post" id="optionItemFrom">
            ...
            <li>
                <select name="isUse" class="required via">
                 		<#list Application['commonStatus'> as item >
                            <option value="${(item.fieldKey)!}"  <#if bean?? && bean.isUse?? && item.fieldKey == bean.isUse?string >
